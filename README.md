...
* **Калібрування** — програма сама розганяє й гальмує вентилятор, знаходить, на
  ...
  відповідні мінімум, старт і поріг зупинки. Працює у фоні: вікно лишається
  живим і показує прогрес.
* **Безпека**:
...
* **Залізо** — материнські чипи через `hwmon` (Nuvoton, ITE тощо), `amdgpu`,
  ...
  NVIDIA дозволяє керувати вентиляторами не на всіх картах і не в усіх версіях
  драйвера. Якщо драйвер відмовляє, програма каже це прямо й перестає
  намагатись, а не засмічує лог щосекунди. Перевірити свою карту:
  `sudo ./tools/nvidia-diagnose.sh`

### Вентилятори NVIDIA і systemd

Демон працює під root, але юніт свідомо забирає в нього **всі** capabilities —
для запису в PWM вони не потрібні. Драйверу NVIDIA потрібні: без них
`nvmlDeviceSetFanSpeed_v2` відповідає «no permission», і виходить, що
температуру GPU видно, а вентиляторами керувати не можна.

Тому на машинах з NVIDIA інсталятор кладе drop-in
`/usr/lib/systemd/system/fancontrold.service.d/nvidia.conf`, який повертає
capabilities. Це **виміряно**, а не вгадано: `tools/nvidia-diagnose.sh`
перевіряє кожне з дванадцяти налаштувань юніта окремо, і ламає керування рівно
одне. Решта ізоляції лишається чинною.

### Coolbits тут не потрібен

Поширена порада «увімкни `Option "Coolbits" "4"` в `xorg.conf`» стосується
**іншого** шляху. Coolbits читає X-драйвер, і він вмикає керування
вентиляторами через `nvidia-settings` та розширення NV-CONTROL.

Ця програма ходить через **NVML** (`libnvidia-ml.so.1`, `/dev/nvidiactl`) —
окремий API, який X-сервера не питає взагалі. Його й робили для машин без
графіки. Тому:

* на Wayland (типово для KDE Plasma на Fedora) `xorg.conf` нікому читати, і
  Coolbits не дав би нічого;
* `nvidia-settings` на Wayland не може керувати вентиляторами в принципі —
  саме тому тут NVML, а не він;
* якщо `sudo ./tools/nvidia-diagnose.sh` показує `SET WORKS` для звичайного
  root, дозволів драйвера достатньо, і справа в чомусь іншому.

Coolbits знадобиться, лише якщо ви **окремо** користуєтесь `nvidia-settings`
під X11.
...
Номери `pwm1..pwmN` нічого не кажуть про те, де фізично стоїть вентилятор.
Визначити можна об'єктивно — по тахометрах, не заглядаючи в корпус:

```bash
sudo ./tools/identify-fans.sh
```

Скрипт тримає всі канали на низьких обертах, по черзі піднімає один до
максимуму й дивиться, який датчик обертів зреагував. Порожні роз'єми теж
видно. Наприкінці все повертається як було — навіть якщо перервати Ctrl-C.

### Якщо вентилятори не реагують на керування

```bash
sudo ./tools/pwm-check.sh
```

Розрізняє три різні причини, які ззовні виглядають однаково:
...
* **тримається, але оберти не змінюються** — вентилятор ігнорує PWM (типово для
  3-пінової вертушки в роз'ємі, переведеному в режим PWM), або ним керує щось
  інше;
* **оберти ростуть разом зі значенням** — керування працює.

Скрипт робить повний свіп 0 → 255 і показує реакцію на кожному кроці, а також
які параметри має драйвер і що він написав при завантаженні.

#### Драйвер може бути налаштований на чужий чип

У `it87` є параметр `force_id`, який змушує вважати чип іншою моделлю — його
часто радять у інструкціях під конкретні плати. Якщо ID чужий, драйвер бере
чужу карту регістрів: частина каналів працює, решта мовчить або відмовляє
переходити в ручний режим.

На Gigabyte B760 Gaming X AX саме так і було: у `/etc/modprobe.d/it87.conf`
стояло `force_id=0x8628`, хоча чип — IT8689E. З `0x8689` запрацювали всі
зайняті роз'єми.
...
`pwm-check.sh` показує обидва і рахує, скільки каналів реально зреагувало.

Якщо ядровий драйвер чипа не знає — позаядерна версія з ширшою підтримкою:
[frankcrawford/it87](https://github.com/frankcrawford/it87).

#### SELinux і відеокарта NVIDIA

systemd обирає SELinux-домен служби за міткою файлу, який запускає. Скрипт у
venv має мітку `lib_t`, переходу для неї немає, і демон лишався в домені
`init_t`, звідки політика не пускає до `/dev/nvidia*` — NVML відповідав
«no permission», хоч би що послаблювалось у пісочниці. Тому юніт запускає
інтерпретатор (`bin_t`), і демон потрапляє в звичайний `unconfined_service_t`.
`fanctl doctor` показує домен демона.
