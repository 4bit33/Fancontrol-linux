...
#### Драйвер може неправильно впізнати чип

`it87` іноді визначає IT8689E як IT8628E — у них різна карта регістрів, тож
записи потрапляють не туди. Ознака: частина каналів працює, частина відмовляє
перевести в ручний режим, а назва чипа не збігається з тим, що показував
Windows.

Визначення буває **недетермінованим**: на одній машині той самий чип за
однією адресою визначався то як IT8689E, то як IT8628E — і при старті системи
теж по-різному. Перевірити:

```bash
dmesg | grep -i "Found.*chip at"
```

`pwm-check.sh` помічає такий конфлікт сам, а ще рахує, скільки каналів реально
зреагувало: якщо майже жоден — це драйвер, а не плата.

Лікується тим, що драйверу нав'язують правильний ID:

```bash
sudo modprobe -r it87
sudo modprobe it87 force_id=0x8689        # той ID, що показав dmesg
sudo ./tools/pwm-check.sh                 # перевірити

# коли запрацює — зробити постійним:
echo "options it87 force_id=0x8689" | sudo tee /etc/modprobe.d/it87.conf
```

Якщо `it87` вбудований у ядро, те саме йде в командний рядок ядра:
`it87.force_id=0x8689`.

Якщо й це не допомогло — позаядерна версія з ширшою підтримкою плат:
