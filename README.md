...
Найчастіше це стається **після перезавантаження модуля**: при старті системи
чип у відомому стані й визначається правильно, а повторне `modprobe` вже може
дати інший результат. Перевірити:

```bash
dmesg | grep -i "Found.*chip at"
```

Якщо для однієї адреси є два різні ID — правильний той, що при завантаженні
системи. `pwm-check.sh` це помічає сам і попереджає. **Спершу перезавантажся**,
і лише якщо не допомогло — позаядерна версія драйвера з ширшою підтримкою:
