...
CURVE_DESCRIPTIONS = {
    CurveType.GRAPH.value: (
        "Graph",
        "Points you place yourself, joined by straight lines. The one to reach "
    ...
    CurveType.LINEAR.value: (
        "Linear",
        "A straight ramp between two temperatures. The same as a two point "
        "graph, but easier to type in exactly.",
    ),
    CurveType.TARGET.value: (
        "Target temperature",
        "Speeds up while the sensor is above the target and slows down while it "
        "is below, instead of mapping temperature to speed directly.",
    ),
    CurveType.TRIGGER.value: (
        "Trigger",
        "Two speeds with a gap between the thresholds, so the fan does not "
        "oscillate around one temperature.",
    ),
    CurveType.MIX.value: (
        "Mix",
        "Combines other curves - usually the highest of a CPU and a GPU curve, "
        "so case fans follow whichever is hotter.",
    ),
    CurveType.SYNC.value: (
        "Sync",
        "Follows another fan, optionally offset or scaled. Good for keeping a "
...
class NewCurveDialog(QDialog):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self.setWindowTitle("New curve")
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("What kind of curve?"))
...
class CurveEditorDialog(QDialog):
    ...
    def __init__(
        ...
        title, _ = CURVE_DESCRIPTIONS.get(curve.type, (curve.type, ""))
        self.setWindowTitle(f"{curve.name} — {title} curve")
        self.resize(760, 460)
    ...
    def _sensor_combo(self, selected: str) -> QComboBox:
        ...
        if index < 0 and selected:
            ...
            combo.addItem(f"{selected}  (not present)", selected)
            index = combo.count() - 1
    ...
    def _build_form(self) -> None:
        ...
        if isinstance(curve, (GraphCurve, LinearCurve, TargetCurve, TriggerCurve)):
            ...
            self.form.addRow("Temperature source", self.sensor)
        ...
        if isinstance(curve, GraphCurve):
            ...
            self.form.addRow("Speed", self.percent)
            ...
            for widget in (self.min_temperature, self.max_temperature,
                ...
                widget.valueChanged.connect(self._on_field_changed)
            self.form.addRow("Ramp starts at", self.min_temperature)
            self.form.addRow("Ramp ends at", self.max_temperature)
            self.form.addRow("Speed at the start", self.min_percent)
            self.form.addRow("Speed at the end", self.max_percent)
            self._add_smoothing_rows()
            self.graph.set_editable(False)
            ...
            for widget in (self.load_temperature, self.idle_temperature,
                ...
                widget.valueChanged.connect(self._on_field_changed)
            self.form.addRow("Speed up above", self.load_temperature)
            self.form.addRow("Slow down below", self.idle_temperature)
            self.form.addRow("Speed when loaded", self.load_percent)
            self.form.addRow("Speed when idle", self.idle_percent)
            self._add_smoothing_rows()
            self.graph.set_editable(False)
            self.hint.setText(
                "Between the two temperatures the fan keeps whatever speed it "
            ...
            for widget in (self.target_temperature, self.idle_percent, self.load_percent,
                ...
                widget.valueChanged.connect(self._on_field_changed)
            self.form.addRow("Target temperature", self.target_temperature)
            self.form.addRow("Slowest", self.idle_percent)
            self.form.addRow("Fastest", self.load_percent)
            self.form.addRow("Speeds up by", self.step_up)
            self.form.addRow("Slows down by", self.step_down)
            self.form.addRow("Dead band", self.deadband)
            self._add_smoothing_rows()
            self.graph.set_editable(False)
            ...
            for value, label in (
                (MixFunction.MAX.value, "Highest of them"),
                (MixFunction.MIN.value, "Lowest of them"),
                (MixFunction.AVERAGE.value, "Average"),
                (MixFunction.SUM.value, "Sum"),
                (MixFunction.SUBTRACT.value, "First minus the rest"),
            ):
                self.function.addItem(label, value)
            ...
            self.form.addRow("Combine using", self.function)
            ...
            self.form.addRow("Inputs", self.inputs)
            self.graph.set_editable(False)

        elif isinstance(curve, SyncCurve):
            self.control = QComboBox()
            self.control.addItem("— choose a fan —", "")
            ...
            self.form.addRow("Follow", self.control)
            self.form.addRow("Scaled by", self.multiplier)
            self.form.addRow("Offset by", self.offset)
            self.graph.set_editable(False)
    ...
    def _refresh_preview(self) -> None:
        ...
        if samples is None:
            ...
            if isinstance(self.curve, MixCurve):
                self.hint.setText(
                    "A mix curve has no shape of its own — it follows whichever "
                    "of its inputs the chosen function picks."
                )
            elif isinstance(self.curve, TargetCurve):
                self.hint.setText(
                    "A target curve has no fixed shape: its speed depends on how "
                    "long the sensor has been above or below the target, not only "
                    "on the current temperature."
                )
            else:
                self.hint.setText("This curve follows another fan, so it has no shape of its own.")
            return
